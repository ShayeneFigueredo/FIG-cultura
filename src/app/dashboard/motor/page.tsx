    ...
    <div className="max-w-5xl">
      <header className="mb-10">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            <h1 className="text-4xl font-semibold mb-2">Motor Agronômico IA</h1>
            <p className="text-white/60 text-lg">Inteligência artificial para planejamento e recomendação de adubação.</p>
          </div>
        </div>
      </header>

      {analyses.length === 0 ? (
        <div className="border border-white/10 border-dashed rounded-3xl p-12 bg-white/5 text-center">
          <Database className="w-16 h-16 text-white/20 mb-4 mx-auto" />
          <h2 className="text-xl font-medium mb-2">Nenhuma Análise Disponível</h2>
        ...
        <div className="space-y-6">
          <h2 className="text-xl font-medium mb-4">Selecione uma análise para gerar o planejamento:</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {analyses.map(analysis => (
              <Link 
                key={analysis.id} 
                href={`/dashboard/analises/${analysis.id}/planejamento`}
                className="group relative bg-white/5 border border-white/10 p-6 rounded-2xl hover:bg-white/10 hover:border-brand-main/50 transition-all flex flex-col justify-between"
              >
                <div>
                  ...
                  <h3 className="text-lg font-semibold text-white mb-1">{analysis.field.name}</h3>
                  <p className="text-sm text-white/50 mb-4">{analysis.field.property.name}</p>
                </div>
